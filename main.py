...
async def send_fanfik(update: Update, context):
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[{"role": "user", "content": "Напиши еротичний фанфік про Нікіту і Віку, які зустрічаються на вечірці. Включи деталі про їхні емоції,сексуальне бажання та фізичні дотики. Промисел має бути еротичним, інтимним і сповненим чуттєвості. Можна додати елементи романтики та конфлікту, щоб зробити сюжет більш захопливим і закінченим, тобто не великим."}],
    )
   # Перевіряємо, чи є відповідь і чи містить вона дані
