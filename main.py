...
async def send_anekdot(update: Update, context):
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[{"role": "user", "content": "Придумай короткий жарт або анекдот, включаючи одного або кількох з таких персонажів: Даня, Віка(яка закохана в Нікіту, вона психолог, обожнює відповідати так се бон, се бьян, се дроль бо вона вчить французьку), Діма(він дуже сатиричний 17 річний хлопчик), Нікіта(Він гей,який не ходить на навчання, постійно шукає відмазки), Нестор(він дуже спокійний завжди), і Ананас(його звати Анас, він араб). Жарт має бути веселим і креативним, але не обов'язково всі персонажі повинні бути в кожному жарті, стеж за тим щоб анекдот був завершений і смішний."}],
    )
